...
async function runFORECAST() {
  ...
  const now = new Date();
  const botNames = Object.values(empireState.bots).map(b => b.displayName || 'Bot');
...
Generate a week-ahead intelligence briefing. Be concise. Return ONLY valid JSON, no extra text:
{
  "weekOutlook": "BULLISH|BEARISH|RANGING|VOLATILE",
  "confidence": 75,
  "keyEvents": [{"date":"Mon Mar 18","event":"Fed speakers","impact":"MEDIUM"},{"date":"Wed Mar 19","event":"Example","impact":"HIGH"}],
  "sectorRotation": [{"sector":"tech","direction":"INTO","reason":"brief reason"}],
  "riskEvents": ["risk one","risk two"],
  "botRecommendations": [{"bot":"${botNames[0]||'Bot'}","rec":"brief recommendation"}],
  "opportunityWindows": [{"day":"Monday","timeET":"9:30-10:30","note":"brief note"}],
  "summary": "Two sentence executive summary of the week ahead."
}`;

  const result = await callClaude(prompt, 2000);
  if (!result) return;
  try {
    // Robust JSON extraction — handle truncation gracefully
    const match = result.match(/\{[\s\S]*\}/);
    if (!match) { console.error('FORECAST: No JSON found in response'); return; }
    let jsonStr = match[0];
    // Attempt to fix common truncation issues by closing unclosed arrays/objects
    let data;
    try {
      data = JSON.parse(jsonStr);
    } catch (e1) {
      // Try to salvage partial JSON by truncating to last complete field
      const lastComma = jsonStr.lastIndexOf(',"');
      if (lastComma > 0) {
        try { data = JSON.parse(jsonStr.slice(0, lastComma) + '}'); } catch (e2) {}
      }
      if (!data) { console.error('FORECAST parse error — could not salvage:', e1.message); return; }
    }
...
Return JSON: {
  ...
  try {
    const match = result.match(/\{[\s\S]*\}/);
    if (!match) { console.error('WAR ROOM: No JSON found'); return; }
    let data;
    try { data = JSON.parse(match[0]); }
    catch (e1) {
      const lastComma = match[0].lastIndexOf(',"');
      if (lastComma > 0) try { data = JSON.parse(match[0].slice(0, lastComma) + '}'); } catch (e2) {}
      if (!data) { console.error('WAR ROOM parse error:', e1.message); return; }
    }
    const report = { ...data, generatedAt: new Date().toISOString() };
  ...
  } catch (e) { console.error('WAR ROOM error:', e.message); }
}
