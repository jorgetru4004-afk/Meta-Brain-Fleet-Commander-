...
async function fetchBotData(name, url) {
  try {
    const resp = await axios.get(`${url}/api/snapshot`, { timeout: 8000 });
    const d = resp.data;
    ...
    return {
      ...
      heat: d.portfolioHeat || 0,
      pnl: d.totalPnl || 0,
      openPnl: d.openPnl || Object.values(positions).reduce((s, p) => s + (p.unrealizedPnl || 0), 0),
      dailyPnl: d.dailyPnl || 0,
      weeklyPnl: d.weeklyPnl || 0,
      totalTrades,
      totalWins,
      winRate: totalTrades > 0 ? parseFloat((totalWins / totalTrades * 100).toFixed(1)) : 0,
      regime: d.marketRegime || 'UNKNOWN',
